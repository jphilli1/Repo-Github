...
def create_credit_deterioration_chart_v3(
    ...
    if show_both_peer_groups:
        entities_to_plot.extend([99999, 90002])  # Both peer groups
        colors = {
            subject_bank_cert: '#F7A81B',  # Golden yellow for subject bank
            90001: '#5B9BD5',              # Blue for all peers
            90002: '#70AD47'               # Green for selective peers
        }
        entity_names = {
...
def generate_credit_metrics_email_table(
    ...
    """
    Builds the Executive Summary Table.
    Comparisons: MSPBNA vs MSBNA (14565) vs Core PB (90001).
    """
    important_metrics = {
        "ASSET": "Total Assets",
        ...
        "RIC_Constr_NCO_Rate": "Constr. NCO Rate (RIC)",
        "RIC_CRE_NCO_Rate": "CRE NCO Rate (RIC)"
    }
    ...
    msbna = latest_data[latest_data["CERT"] == 14565] # MSBNA (Bank N.A.)
    core_pb = latest_data[latest_data["CERT"] == 90001] # Core PB

    if idb.empty: return None, None

    idb = idb.iloc[0]
    msbna = msbna.iloc[0] if not msbna.empty else pd.Series(dtype=float)
    core_pb = core_pb.iloc[0] if not core_pb.empty else pd.Series(dtype=float)
    ...
    for code, disp in important_metrics.items():
        ...
        v_msbna = msbna.get(code, np.nan)
        v_core  = core_pb.get(code, np.nan)

        # Diffs: (Subject - Benchmark)
        d_msbna = (v_idb - v_msbna) if (not pd.isna(v_msbna) and not pd.isna(v_idb)) else np.nan
        d_core  = (v_idb - v_core)  if (not pd.isna(v_core) and not pd.isna(v_idb)) else np.nan

        if code in ["ASSET", "LNLS"]:
            fmt = _fmt_money_millions; fmt_diff = _fmt_money_millions_with_sign
        elif "RRI" in code:
            fmt = lambda x: f"{x:.2f}x" if pd.notna(x) else "N/A"
            fmt_diff = lambda x: f"{x:+.2f}x" if pd.notna(x) else "N/A"
        else:
            fmt = _fmt_percent_auto; fmt_diff = _fmt_percent_auto
        ...
        rows.append({
            ...
            "Diff vs MSBNA": fmt_diff(d_msbna),
            "Diff vs Core PB": fmt_diff(d_core),
        })

    df = pd.DataFrame(rows)
    # Update HTML Headers in-line logic
    html = generate_html_email_table_dynamic(df, latest_date)
    return html, df

def generate_html_email_table_dynamic(df: pd.DataFrame, report_date: datetime) -> str:
    """Helper to generate HTML with dynamic headers matching the DF columns"""
    cols = df.columns.tolist()

    html = f"""
    <html><head><style>
        body {{ font-family: Arial, sans-serif; background-color: #f5f5f5; }}
        .email-container {{ background-color: white; padding: 20px; border-radius: 8px; max-width: 1000px; margin: 0 auto; }}
        table {{ width: 100%; border-collapse: collapse; margin: 20px 0; font-size: 12px; }}
        th {{ background-color: #002F6C; color: white; padding: 10px; border: 1px solid #2c3e50; }}
        td {{ padding: 8px; text-align: center; border: 1px solid #bdc3c7; }}
        .metric-name {{ text-align: left !important; font-weight: bold; color: #2c3e50; }}
        .subject-value {{ background-color: #E6F3FF; font-weight: bold; color: #002F6C; }}
        .positive {{ color: #d32f2f; font-weight: 600; }}
        .negative {{ color: #388e3c; font-weight: 600; }}
    </style></head><body>
    <div class="email-container">
        <h2 style="text-align:center; color:#002F6C;">MSPBNA Credit Risk Monitor</h2>
        <p style="text-align:center;">Report Date: {report_date.strftime('%Y-%m-%d')}</p>
        <table><thead><tr>"""
    ...
    for _, row in df.iterrows():
        ...
        for c in cols:
            ...
            if "Diff" in c and "N/A" not in str(val):
                # Simple heuristic: red if > 0 (higher risk/metrics)
                try:
                    num = float(str(val).replace('+','').replace('%','').replace('x','').replace('$','').replace('M','').replace(',',''))
                    if any(x in row['Metric'] for x in ['Rate', 'Index', 'Coverage']):
                         cls = 'class="positive"' if num > 0 else 'class="negative"'
                except: pass

            if c == "Metric": val = f"<b>{val}</b>"; cls = 'class="metric-name"'
            if c == "MSPBNA": cls = 'class="subject-value"'
...
def generate_reports(
    ...
    try:
        ...
        fig, ax = create_credit_deterioration_chart_ppt(
            ...
            line_metric="Nonaccrual_to_Gross_Loans_Rate",
            # ENTITIES: MSPBNA (34221), MSBNA (14565), Core PB (90001)
            bar_entities=[subject_bank_cert, 14565, 90001],
            line_entities=[subject_bank_cert, 14565, 90001],
            # STYLE ARGS (Restored)
            figsize=(14, 7),
            title_size=20,
            axis_label_size=14,
            ...
            custom_title="Asset Quality: NCOs (Bars) vs Nonaccruals (Lines)",
            save_path=str((Path(excel_file).parent / "Peers" /  f"{base}_NCO_NA_CHART_{stamp}.png")))
        ...
        plot_scatter_dynamic(
            ...
            peer_avg_cert_primary=90003,  # UPDATED: Plot ALL_PEERS (90003)
            peer_avg_cert_alt=90001,      # Secondary: Core PB (90001)
        ...
        print(f"✓ Scatter saved: Coverage vs Nonaccrual (All Peers)")

        # 2. Wealth Strategy
        s2_path = peers_dir / f"{base}_scatter_rri_resi_{stamp}.png"
        plot_scatter_dynamic(
            df=rolling8q_df,
            x_col="RIC_CRE_Risk_Adj_Coverage",
            y_col="RIC_CRE_Years_of_Reserves",
            subject_cert=subject_bank_cert,
            peer_avg_cert_primary=90003,  # UPDATED: Plot ALL_PEERS (90003)
            peer_avg_cert_alt=90001,      # Secondary: Core PB
            figsize=(9, 9),
            title_size=16,
            economist_style=True,
            save_path=str(s2_path)
        )
        print(f"✓ Scatter saved: RRI Residential Strategy (All Peers)")
...
def create_credit_deterioration_chart_ppt(
    ...
    MS_BLUE = "#002F6C"    # MSPBNA
    MS_LIGHT = "#4C78A8"   # MSBNA / Affiliates
    CORE_GOLD = "#F7A81B"  # Core Peers
    PEER_GRAY = "#7F8C8D"  # All Peers

    # Default entities if None
    default_entities = [subject_bank_cert, 14565, 90001] # MSPBNA, MSBNA, Core PB
...
def plot_scatter_dynamic(
    ...
    x_col: str,
    y_col: str,
    subject_cert: int = 34221,       # UPDATED: MSPBNA
    peer_avg_cert_primary: int = 90001, # UPDATED: Core PB
    peer_avg_cert_alt: int = 90002,     # UPDATED: MS+Wealth
    ...
    if identify_outliers and outliers_topn > 0:
        ...
        if not idb.empty and int(idb.index[0]) in top_idx:
            top_idx = [i for i in top_idx if i != int(idb.index[0])][:1]
        else:
