...
class BankMetricsProcessor:
    ...
    def _safe_divide(self, numerator, denominator, fill_value=0.0):
        """
        Safely divide two values/series, handling zeros and edge cases.

        Args:
            numerator: Numeric value or Series
            denominator: Numeric value or Series
            fill_value: Value to use for division by zero (default: 0.0)

        Returns:
            Result of division with zeros/NaNs replaced by fill_value

        Examples:
            >>> _safe_divide(100, 50)  # Returns 2.0
            >>> _safe_divide(100, 0)   # Returns 0.0
            >>> _safe_divide(Series([10, 20]), Series([5, 0]))  # Returns Series([2.0, 0.0])
        """
        ...
        if isinstance(denominator, pd.Series):
            # Replace zeros with NaN for division, then fill
            denom_safe = denominator.replace(0, np.nan)
            result = (numerator / denom_safe).fillna(fill_value)
            return result

        # Handle mixed scalar/Series
        ...
        try:
            ...
            logger.warning(f"Division failed: {e}. Returning fill_value={fill_value}")
            if isinstance(numerator, pd.Series):
                return pd.Series(fill_value, index=numerator.index)
            return fill_value
    ...
    def create_derived_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for cert, group in df_processed.groupby('CERT'):
            ...
            df_processed.loc[idx, 'Total_Past_Due_to_Book'] = self._safe_divide(past_due, loans)
            df_processed.loc[idx, 'Allowance_to_Gross_Loans_Rate'] = self._safe_divide(total_acl, loans)
            df_processed.loc[idx, 'Nonaccrual_to_Gross_Loans_Rate'] = self._safe_divide(nonaccrual, loans)
            df_processed.loc[idx, 'NPL_to_Gross_Loans_Rate'] = self._safe_divide(nplloans, loans)
            df_processed.loc[idx, 'NPL_to_Past_Due'] = self._safe_divide(nplloans, past_due)
            df_processed.loc[idx, 'NPL_to_ACL'] = self._safe_divide(nplloans, total_acl)
    ...
    def calculate_ttm_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for cert, group in df.groupby('CERT'):
            ...
            if len(bank_df) >= 4:
                ...
                bank_df['TTM_NCO_Rate'] = self._safe_divide(bank_df['NTLNLS_Q'].rolling(window=4).sum(), avg_loans_ttm)
                bank_df['TTM_PD30_Rate'] = self._safe_divide(bank_df['P3LNLS'].rolling(window=4).mean(), avg_loans_ttm)
                bank_df['TTM_PD90_Rate'] = self._safe_divide(bank_df['P9LNLS'].rolling(window=4).mean(), avg_loans_ttm)
                ...
                for cat_name, cat_details in LOAN_CATEGORIES.items():
                    ...
                    bank_df[f'IDB_{cat_name}_TTM_PD30_Rate'] = self._safe_divide(avg_pd30_ttm, avg_cat_balance_ttm)

                    pd90_balance = self._get_series(bank_df, cat_details.get('pd90', []))
                    avg_pd90_ttm = pd90_balance.rolling(window=4).mean()
                    bank_df[f'IDB_{cat_name}_TTM_PD90_Rate'] = self._safe_divide(avg_pd90_ttm, avg_cat_balance_ttm)

                    nco_balance = self._get_series(bank_df, cat_details.get('nco', []))
                    avg_nco_ttm = nco_balance.rolling(window=4).mean()
                    bank_df[f'IDB_{cat_name}_TTM_NCO_Rate'] = self._safe_divide(avg_nco_ttm, avg_cat_balance_ttm)
...
class BankPerformanceDashboard:
    ...
    def _create_peer_composite(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        peers_excl_flagstar_valley = [cert for cert in self.config.peer_bank_certs if cert not in [3510, 3511, 628, 7213,17281]]
        ...
        if not peer_df_selective.empty:
            with warnings.catch_warnings():
                ...
                peer_avg_selective['NAME'] = "Peers' Average (Ex. BOFA,WF,JPM,CITI, & RBC)"
...
def load_config() -> DashboardConfig:
    ...
    peer_certs_str = os.getenv('PEER_CERTS', '33124,20188, 57450, 57565,20188 ')
    peer_certs = [int(x.strip()) for x in peer_certs_str.split(',') if x.strip()]

    logger.info(f"Configuration loaded: Subject CERT={subject_cert}, {len(peer_certs)} peer banks")
