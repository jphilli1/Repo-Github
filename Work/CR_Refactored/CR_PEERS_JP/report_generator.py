    ...
    with pd.ExcelFile(xlsx_path) as xls:
        ...
        for cand in ["FRED_Data", "FRED_data", "FRED", "fred"]:
            if cand in xls.sheet_names:
                ...
                print(f"fred.columns.tolists(): ", fred.columns.tolist())
                break
        else:
        ...
        for cand in ["FRED_Descriptions", "FRED_Dictionary", "FRED Meta"]:
            if cand in xls.sheet_names:
                ...
                print(f"desc.columns.tolists(): ", desc.columns.tolist())
                break
        else:
...
def generate_reports(
    ...
    try:
        ...
        with pd.ExcelFile(excel_file) as xls:
            ...
            print(f"proc_df_with_peers.columns.tolists(): ", proc_df_with_peers.columns.tolist())
            ...
            print(f"rolling8q_df.columns.tolists(): ", rolling8q_df.columns.tolist())
            if "CERT" in rolling8q_df.columns:
                rolling8q_df["CERT"] = pd.to_numeric(rolling8q_df["CERT"], errors="coerce").astype("Int64")

            metric_descriptions = pd.read_excel(xls, sheet_name="Data_Dictionary_Audit") \
                if "Data_Dictionary_Audit" in xls.sheet_names else None
            print(f"metric_descriptions.columns.tolists(): ", metric_descriptions.columns.tolist())
