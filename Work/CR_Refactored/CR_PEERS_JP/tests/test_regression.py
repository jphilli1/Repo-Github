...
# Add project root and src subdirectories to path for restructured repo layout
_REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, _REPO_ROOT)
sys.path.insert(0, os.path.join(_REPO_ROOT, "src", "data_processing"))
sys.path.insert(0, os.path.join(_REPO_ROOT, "src", "reporting"))
sys.path.insert(0, os.path.join(_REPO_ROOT, "src", "local_macro"))
...
def test_report_generator_uses_canonical_resi_names():
    ...
    import re
    report_path = os.path.join(_REPO_ROOT, "src", "reporting", "report_generator.py")
...
def test_workbook_includes_audit_sheets():
    ...
    src_path = os.path.join(_REPO_ROOT, "src", "data_processing", "MSPBNA_CR_Normalized.py")
...
def test_no_stale_19977_in_report_generator():
    ...
    src_path = os.path.join(_REPO_ROOT, "src", "reporting", "report_generator.py")
    with open(src_path, "r", encoding="utf-8") as f:
        source = f.read()
...
class TestDateOnlyNaming(unittest.TestCase):
    ...
    def test_mspbna_no_hhmmss_in_filename(self):
        ...
        src_path = Path(_REPO_ROOT) / "src" / "data_processing" / "MSPBNA_CR_Normalized.py"
    ...
    def test_report_generator_no_duplicate_date_stamp(self):
        ...
        src_path = Path(_REPO_ROOT) / "src" / "reporting" / "report_generator.py"
        src = src_path.read_text(encoding="utf-8")
...
class TestClaudeMDLogging(unittest.TestCase):
    ...
    def test_claude_md_mentions_csv_logging(self):
        ...
        md_path = Path(_REPO_ROOT) / "CLAUDE.md"
...
class TestDeprecatedNormalizedComparison(unittest.TestCase):
    ...
    def test_deprecated_stub_exists(self):
        ...
        src = Path(_REPO_ROOT) / "src" / "reporting" / "report_generator.py"
...
class TestImportSafety(unittest.TestCase):
    ...
    def test_no_import_time_value_error(self):
        ...
        src = Path(_REPO_ROOT) / "src" / "data_processing" / "MSPBNA_CR_Normalized.py"
        source = src.read_text(encoding="utf-8")
...
class TestInferFreqFromIndex(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        import re
        src = (Path(_REPO_ROOT) / "src" / "data_processing" / "MSPBNA_CR_Normalized.py").read_text(encoding="utf-8")
...
class TestCsvLogSeverityClassification(unittest.TestCase):
    ...
    def test_tee_to_logger_has_classify_level(self):
        ...
        src = Path(_REPO_ROOT) / "logging_utils.py"
        source = src.read_text(encoding="utf-8")
...
class TestACLRatioSemanticIntegrity(unittest.TestCase):
    ...
    def test_metric_registry_has_share_entries(self):
        ...
        src = Path(_REPO_ROOT) / "metric_registry.py"
        source = src.read_text(encoding="utf-8")
...
class TestConsistencyPass(unittest.TestCase):
    ...
    def test_all_risk_adj_coverage_in_x_format(self):
        ...
        src = Path(_REPO_ROOT) / "src" / "reporting" / "report_generator.py"
        source = src.read_text(encoding="utf-8")
        main_src = Path(_REPO_ROOT) / "src" / "data_processing" / "MSPBNA_CR_Normalized.py"
    ...
    def test_enrichment_status_codes_match_code_and_docs(self):
        ...
        mapper = Path(_REPO_ROOT) / "case_shiller_zip_mapper.py"
        mapper_src = mapper.read_text(encoding="utf-8")
        claude = Path(_REPO_ROOT) / "CLAUDE.md"
...
class TestEnrichmentStatusCodes(unittest.TestCase):
    ...
    def test_status_codes_defined(self):
        ...
        src = Path(_REPO_ROOT) / "case_shiller_zip_mapper.py"
        source = src.read_text(encoding="utf-8")
...
class TestCanonicalRenderingArchitecture(unittest.TestCase):
    ...
    def test_no_merge_conflicts_in_test_regression(self):
        ...
        import re
        src = Path(_REPO_ROOT) / "tests" / "test_regression.py"
        text = src.read_text()
...
class TestWealthPeersInclusion(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls._src = (Path(_REPO_ROOT) / "src" / "reporting" / "report_generator.py").read_text()
...
class TestFootballFieldKRI(unittest.TestCase):
    ...
    def test_football_field_uses_nested_bands(self):
        ...
        src = Path(_REPO_ROOT) / "executive_charts.py"
        content = src.read_text(encoding="utf-8")
