...
from enhanced_dictionary_validator import EnhancedDataDictionaryClient
# Import field definitions
try:
    from FDIC_FFIEC_Series_Key import (
        FDIC_FIELDS_TO_FETCH, FRED_SERIES_TO_FETCH, LOAN_CATEGORIES,
        DERIVED_FIELD_DEFINITIONS as FDIC_FIELD_DESCRIPTIONS
    )
except ImportError:
...
def fetch_mdrm_definition(code: str) -> str:
    """
    Fetches the official Item Name for a Call Report code (e.g., RIAD4655)
    from the Federal Reserve MDRM website.
    """
    try:
        # The MDRM website URL pattern for item details
        url = f"https://www.federalreserve.gov/apps/mdrm/itemdetail?confId=CALL&itemCode={code}"

        # We use a simple request (add headers to look like a browser)
        headers = {'User-Agent': 'Mozilla/5.0'}
        response = requests.get(url, headers=headers, timeout=5)

        if response.status_code == 200:
            # Simple string parsing to find the "Item Name"
            # The page usually has: <th>Item Name:</th> <td> ... </td>
            if "Item Name:" in response.text:
                start_marker = "Item Name:</th>"
                start_index = response.text.find(start_marker)
                if start_index != -1:
                    # Extract the table cell content
                    snippet = response.text[start_index:]
                    # Find the first <td> tag content
                    name_start = snippet.find("<td>") + 4
                    name_end = snippet.find("</td>")
                    clean_name = snippet[name_start:name_end].strip()
                    return clean_name
...
class MDRMClient:
    """
    A robust client to validate Call Report codes against the
    Federal Reserve's Micro Data Reference Manual (MDRM).
    """
    BASE_URL = "https://www.federalreserve.gov/apps/mdrm/data-dictionary"
    ...
    def lookup_code(self, full_code: str) -> str:
        """
        Parses a code (e.g., 'RIAD4655') and fetches its official definition.
        """
        if full_code in self._cache:
            return self._cache[full_code]

        # 1. Parse Code (Split Mnemonic from Number)
        # Regex: Capture letters (Mnemonic) and numbers (Item Code)
        match = re.match(r"([A-Z]+)(\d+)", full_code)
        if not match:
            return "Invalid Code Format"

        mnemonic, item_code = match.groups()

        # 2. Construct Search URL
        # We search by "Item Code" (the number) because Mnemonics vary (RCFD vs RCON)
        params = {
            "search_string": item_code,
            "search_type": "mnemonic_code"
        }
        ...
        try:
            ...
            # 3. Parse with Pandas (Robust Table Scraping)
            dfs = pd.read_html(io.StringIO(response.text))

            # The search result usually contains a table with columns:
            # 'Mnemonic', 'Item Code', 'Item Name', 'Confidential'
            for df in dfs:
                if "Item Name" in df.columns and "Item Code" in df.columns:
                    # Filter for the row that matches our specific item code
                    # Convert to string to ensure matching works
                    df['Item Code'] = df['Item Code'].astype(str)
                    match_row = df[df['Item Code'] == item_code]

                    if not match_row.empty:
                        # Grab the first match's name
                        official_name = match_row.iloc[0]["Item Name"]
                        self._cache[full_code] = official_name
                        return official_name

            return "Definition Not Found (No Table Match)"

        except ValueError:
            # pd.read_html throws ValueError if no tables found
            return "No Definition Table Found"
...
class MDRMValidator:
    """
    Validates Call Report codes against the Official Federal Reserve MDRM Dictionary.
    Downloads the full dictionary CSV to ensure 100% accuracy.
    """
    # Official FFIEC MDRM Dictionary Download URL (Public)
    MDRM_CSV_URL = "https://www.federalreserve.gov/apps/mdrm/download/MDRM.zip"

    def __init__(self, output_dir: str):
        self.output_dir = Path(output_dir)
        self.dictionary_path = self.output_dir / "MDRM_Official_Dictionary.csv"
        self._mdrm_df = None

    def load_or_fetch_dictionary(self):
        """Downloads and loads the official MDRM dictionary."""
        if self.dictionary_path.exists():
            # Check if file is older than 30 days, if so, re-download
            file_age = time.time() - self.dictionary_path.stat().st_mtime
            if file_age < (30 * 86400):
                print(f"  ✓ Loading cached MDRM dictionary ({self.dictionary_path.name})...")
                self._mdrm_df = pd.read_csv(self.dictionary_path, low_memory=False)
                return

        print("  ⬇ Downloading Official MDRM Dictionary from Federal Reserve...")
        try:
            # Download ZIP
            resp = requests.get(self.MDRM_CSV_URL, timeout=30)
            if resp.status_code == 200:
                with zipfile.ZipFile(io.BytesIO(resp.content)) as z:
                    # Look for the main text file (usually MDRM.csv or MDRM.txt)
                    file_name = [f for f in z.namelist() if f.lower().endswith(('.txt', '.csv'))][0]
                    with z.open(file_name) as f:
                        # Load into Pandas (It's usually Tab or Comma delimited)
                        df = pd.read_csv(f, sep=None, engine='python', encoding='latin1')

                        # Normalize Columns (Mnemonic, Item Code, Item Name)
                        df.columns = [c.upper().strip() for c in df.columns]

                        # Create a "Full Code" column (e.g., RIAD + 4655)
                        # MDRM usually has 'MNEMONIC' and 'ITEM CODE'
                        if 'MNEMONIC' in df.columns and 'ITEM CODE' in df.columns:
                            df['FULL_CODE'] = df['MNEMONIC'] + df['ITEM CODE'].astype(str)

                        # Save to local CSV for next time
                        df.to_csv(self.dictionary_path, index=False)
                        self._mdrm_df = df
                        print("  ✓ Download complete and cached.")
            else:
                print(f"  ⚠ Failed to download MDRM (HTTP {resp.status_code}). Validation will be limited.")

        except Exception as e:
            print(f"  ⚠ Error fetching MDRM: {e}")

    def validate_code(self, full_code: str) -> str:
        """Returns the Official Definition or 'Not Found'."""
        if self._mdrm_df is None:
            return "Validator Not Loaded"

        # Lookup
        match = self._mdrm_df[self._mdrm_df['FULL_CODE'] == full_code]
        if not match.empty:
            # Return the Item Name
            return match.iloc[0].get('ITEM NAME', 'Description Missing')

        return "Definition Not Found in Official MDRM"
...
class FFIECBulkLoader:
    ...
    def heal_dataset(self, df_fdic: pd.DataFrame, peer_certs: set) -> pd.DataFrame:
        ...
        dates = sorted(df_fdic['REPDTE'].unique(), reverse=True)[:60]
        ffiec_frames = []
...
class BankMetricsProcessor:
    ...
    def create_derived_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for cert, group in df_processed.groupby('CERT'):
            ...
            if 'NTREMULT_Q' in group.columns and 'NTRENROT_Q' in group.columns:
                group['Inv_CRE_NCO_TTM'] = (group['NTREMULT_Q'] + group['NTRENROT_Q']).rolling(window=4,
                                                                                               min_periods=1).sum()
            else:
                group['Inv_CRE_NCO_TTM'] = 0.0
        ...
        # 1. C&I (Commercial & Industrial) - Corrected Mapping & Logic
        # -------------------------------------------------------------------------

        # A. Calculate TRUE Total C&I NCOs (Summing components as requested)
        # Charge-offs: US (4645) + Non-US (4646). Fallback to Total (4638).
        ci_co_granular = df_processed.get('RIAD4645', 0).fillna(0) + df_processed.get('RIAD4646', 0).fillna(0)
        ci_co_total = best_of(df_processed, ['RIAD4638']).fillna(0)

        # Use granular if available, otherwise total.
        # (Fixes the bug where we mapped 4608/Recoveries to Chargeoffs)
        actual_ci_chargeoffs = np.where(ci_co_granular > 0, ci_co_granular, ci_co_total)

        # Recoveries: US (4617) + Non-US (4618). Fallback to Total (4608).
        ci_rec_granular = df_processed.get('RIAD4617', 0).fillna(0) + df_processed.get('RIAD4618', 0).fillna(0)
        ci_rec_total = best_of(df_processed, ['RIAD4608']).fillna(0)
        actual_ci_recoveries = np.where(ci_rec_granular > 0, ci_rec_granular, ci_rec_total)

        total_ci_nco = actual_ci_chargeoffs - actual_ci_recoveries

        # B. Apply Proportional Exclusion (The "Peer-Safe" Logic)
        # If we kept the Balance (SBL), we must keep the NCO.
        # If we excluded the Balance (Corporate), we must exclude the NCO.

        # Calculate Exclusion Ratio: Excluded_CI_Bal / Total_CI_Bal
        total_ci_bal = best_of(df_processed, ['LNCI', 'RCON1763', 'RCFD1763']).replace(0, np.nan)
        excl_ci_bal = norm_cols.get('Excl_CI_Balance', 0)

        exclusion_ratio = (excl_ci_bal / total_ci_bal).fillna(0).clip(upper=1.0)

        # Final Calculation
        # For MSPBNA: Ratio is ~0% (SBL kept) -> Excl NCO is ~0 -> Norm NCO = $41M.
        # For Peers: Ratio is ~90% (Corp excl) -> Excl NCO is High -> Norm NCO is Low.
        norm_cols['Excl_CI_NCO_YTD'] = total_ci_nco * exclusion_ratio

        # 2. ADC (Construction) - Item 1.a
        # Try Aggregate RIAD4658 first, else sum 1.a.1 (C891) and 1.a.2 (C893)
        adc_chargeoffs = best_of(df_processed, ['RIAD4658']).fillna(0)
        if (adc_chargeoffs == 0).all():
            adc_chargeoffs = df_processed.get('RIADC891', 0).fillna(0) + df_processed.get('RIADC893', 0).fillna(0)

        adc_recoveries = best_of(df_processed, ['RIAD4659']).fillna(0)
        if (adc_recoveries == 0).all():
            adc_recoveries = df_processed.get('RIADC892', 0).fillna(0) + df_processed.get('RIADC894', 0).fillna(0)

        norm_cols['Excl_ADC_NCO_YTD'] = adc_chargeoffs - adc_recoveries

        # 3. Credit Cards - Item 5.a
        ...
        # 4. Auto Loans - Item 5.b (CORRECTED from RIADK205 to RIADK129)
        auto_chargeoffs = best_of(df_processed, ['RIADK129']).fillna(0)
        auto_recoveries = best_of(df_processed, ['RIADK133']).fillna(0)
        norm_cols['Excl_Auto_NCO_YTD'] = auto_chargeoffs - auto_recoveries

        # 5. Other Consumer - Item 5.c (NEW Exclusion)
        oth_cons_chargeoffs = best_of(df_processed, ['RIADK205']).fillna(0)
        oth_cons_recoveries = best_of(df_processed, ['RIADK206']).fillna(0)
        excl_oth_cons_nco = oth_cons_chargeoffs - oth_cons_recoveries

        # 6. Ag Loans - Item 3 (CORRECTED from RIAD4635 to RIAD4655)
        # This was the root cause: RIAD4635 is TOTAL NCOs. RIAD4655 is Ag.
        ...
        # Use calculated value if available (non-zero), otherwise use fallback
        norm_cols['Excl_Ag_NCO_YTD'] = np.where(ag_nco_calc != 0, ag_nco_calc, ag_nco_fallback)

        # 7. NDFI / All Other Loans - Item 7 (The missing $7,000)
        # This captures "Loans to foreign govts" (Item 6) + "All other loans" (Item 7)
        other_chargeoffs = (
                best_of(df_processed, ['RIAD4643']).fillna(0) +  # Foreign Govts
                best_of(df_processed, ['RIAD4644']).fillna(0)  # All Other (The $7k)
        )
        other_recoveries = (
                best_of(df_processed, ['RIAD4627']).fillna(0) +
                best_of(df_processed, ['RIAD4628']).fillna(0)
        )
        # Combine NDFI/Other/OtherConsumer into one "NDFI+" bucket
        norm_cols['Excl_NDFI_NCO_YTD'] = (other_chargeoffs - other_recoveries) + excl_oth_cons_nco

        # CORRECTED CODE
        ag_chargeoffs = best_of(df_processed, ['RIAD4655']).fillna(0)  # <--- Correct Ag Charge-off Code
        ag_recoveries = best_of(df_processed, ['RIAD4665']).fillna(0)  # <--- Correct Ag Recovery Code
        ag_nco_calc = ag_chargeoffs - ag_recoveries
        ag_nco_fallback = best_of(df_processed, ['NTAG']).fillna(0)
        norm_cols['Excl_Ag_NCO_YTD'] = np.where(ag_nco_calc != 0, ag_nco_calc, ag_nco_fallback)
        ...
        total_na = df_processed['Total_Nonaccrual'] if 'Total_Nonaccrual' in df_processed.columns else pd.Series(0.0,
        ...
        # ==============================================================================
        # 1. Total ACL
        total_acl = best_of(df_processed, ['Total_ACL', 'RCFD3123', 'RIAD3123']).fillna(0)

        # 2. Exclude specific segments (Add C&I to this list)
        res_adc = df_processed.get('RIC_Constr_ACL', 0).fillna(0)
        res_cc = df_processed.get('RIC_Card_ACL', 0).fillna(0)
        res_oth = df_processed.get('RIC_OthCons_ACL', 0).fillna(0)

        # [FIX] Exclude C&I Reserves (RIC_Comm_ACL) to match the Excluded Loans
        res_ci = df_processed.get('RIC_Comm_ACL', 0).fillna(0)

        # 3. Calculate Norm ACL
        norm_acl_balance = total_acl - (res_adc + res_cc + res_oth + res_ci)
...
class ExcelOutputGenerator:
    ...
    def __init__(self, config: 'DashboardConfig'):
        self.config = config
        # Generate comprehensive data dictionary from multiple sources
        print("\n" + "=" * 80)
        print("  GENERATING ENHANCED DATA DICTIONARY")
        print("=" * 80)

        # Initialize enhanced dictionary client
        dict_client = EnhancedDataDictionaryClient(
            output_dir=str(self.config.output_dir),  # Changed: self.config.output_dir
            excel_dict_path=r"C:\Users\jmsph\PyCharmMiscProject\Documentation\All Financial Reports.xlsx"
        )

        # Load all available dictionaries
        dict_client.load_all_dictionaries()

        # Generate comprehensive dictionary
        self.fdic_desc_df = dict_client.generate_comprehensive_dictionary()

        # Ensure compatibility with existing sheet styling
        # Rename columns to match expected format (if needed)
        if 'Metric_Code' in self.fdic_desc_df.columns:
            # Already in correct format
            pass
        else:
            # Legacy fallback (shouldn't happen with new code)
            print("⚠️  Warning: Using legacy dictionary format")
            from FDIC_FFIEC_Series_Key import DERIVED_FIELD_DEFINITIONS
            self.fdic_desc_df = pd.DataFrame.from_dict(
                DERIVED_FIELD_DEFINITIONS, orient='index'
            ).reset_index()
            ...
            self.fdic_desc_df['Source_of_Truth'] = 'Local DERIVED_FIELD_DEFINITIONS'

        print(f"✓ Generated data dictionary with {len(self.fdic_desc_df):,} entries")
        print("=" * 80 + "\n")
        ...
        with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
            ...
            def _style_metric_descriptions_sheet(self, writer):
                ...
                column_widths = {
                    ...
                    'B': 40,  # Metric_Name
                    'C': 80,  # Description
                    'D': 35,  # Source_of_Truth (NEW)
                    'E': 12,  # Data_Type (NEW)
                    'F': 15,  # Schedule (NEW)
                    'G': 10,  # In_MDRM (NEW)
                    'H': 12,  # In_FDIC_API (NEW)
                    'I': 12,  # In_FDIC_XML (NEW)
                    'J': 12,  # In_Local_Defs (NEW)
                }
                ...
                # Color-code by Source_of_Truth (Column D)
                source_colors = {
                    'MDRM Dictionary (Federal Reserve)': 'C6E0B4',  # Light green
                    'FDIC API Fields': 'BDD7EE',  # Light blue
                    'FDIC XML Dictionary': 'F8CBAD',  # Light orange
                    'Local DERIVED_FIELD_DEFINITIONS': 'FFE699',  # Light yellow
                    'NOT FOUND': 'FFC7CE'  # Light red
                }

                # Apply conditional formatting based on source
                for row_idx, row in enumerate(ws.iter_rows(min_row=2, max_row=ws.max_row), start=2):
                    source_cell = row[3]  # Column D (Source_of_Truth)
                    source_value = source_cell.value

                    if source_value in source_colors:
                        fill_color = source_colors[source_value]
                ...
                print(f"  ✓ Styled {sheet_name} sheet with enhanced formatting")
...
def main():
    ...
    try:
        ...
        def generate_dictionary_coverage_report(dict_client: EnhancedDataDictionaryClient,
                                                output_dir: Path):
            print("\n" + "=" * 80)
            print("  GENERATING DICTIONARY COVERAGE REPORT")
            print("=" * 80)

            comprehensive_df = dict_client.generate_comprehensive_dictionary()

            # Coverage statistics
            total_fields = len(comprehensive_df)

            stats = {
                'Total Unique Fields': total_fields,
                'Found in MDRM': (comprehensive_df['In_MDRM'] == '✓').sum(),
                'Found in FDIC API': (comprehensive_df['In_FDIC_API'] == '✓').sum(),
                'Found in FDIC XML': (comprehensive_df['In_FDIC_XML'] == '✓').sum(),
                'Found in Local Defs': (comprehensive_df['In_Local_Defs'] == '✓').sum(),
                'Not Found Anywhere': (comprehensive_df['Source_of_Truth'] == 'NOT FOUND').sum()
            }
            ...
            coverage_df = pd.DataFrame([
                {
                    'Source': key,
                    'Count': val,
                    'Coverage %': f"{(val / total_fields) * 100:.1f}%"
                }
                for key, val in stats.items()
            ])

            # Save report
            report_path = output_dir / 'Dictionary_Coverage_Report.csv'
