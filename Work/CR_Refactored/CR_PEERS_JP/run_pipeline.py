...
def main():
    ...
    if args.step in ("1", "both"):
        ...
        if not os.environ.get("HUD_USER_TOKEN"):
            print("[pipeline] WARNING: HUD_USER_TOKEN not set — "
                  "Case-Shiller ZIP enrichment will be skipped")
        if missing:
            print(f"[pipeline] ERROR: Required environment variable(s) missing: "
                  f"{', '.join(missing)}")
            print("  Set them in .env or export before running. Example:")
            for var in missing:
